<script lang="ts">
  ...
  import { twMerge } from 'tailwind-merge';
	import type { HtmlInputTypes } from '$lib/types/html/html-types.js';
  ...
  export let id: $$Props['id'] = Utils.genUuid();
...
<div class={twMerge(divClasses[style], classDiv)}>
  <input 
    {id} 
    {...$$restProps} 
    bind:value on:blur on:change on:click on:focus on:input on:keydown on:keypress on:keyup on:mouseenter on:mouseleave on:mouseover on:paste {...{ type }} placeholder=" " class={twMerge(inputClasses[style], inputColorClasses[color], inputSizes[style][size], classInput)} 
    bind:this={inputEl}
    />
